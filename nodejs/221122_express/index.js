...
//post 요청 받기 == post 메소드 사용
app.post("/postforms", (req,res)=>{ // 원래 라우터였던 form에서 postform라우터로 이동한 다음 send안에 문자열이 보이게 된다.
    console.log(req.body); // 클라이언트가 서버에 보내는 요청을 body에 받아온다. (get과 차이)
    res.send("post 요청성공!");
})
...
app.get('/img', (req,res)=> {
    res.render("2index", {
        data : ["/static/img/cat1.jpeg", "static/img/cat2.jpeg", "static/img/cat3.jpg"]
    });
})
