...
// 액션 타입(문자열) 각각의 기능을 위한 액션 type 설정
const CHECK = 'mbti/CHECK'; //mbti 모듈에서 온 CHECK
const NEXT = 'mbti/NEXT';
const RESET = 'mbti/RESET';

// 액션 생성 함수 -- 외부에서 쓸 것이므로 export
// payload -> 선택에 다른 결과 값 result 전달 필요
export function check(result) {
  return {
    type: CHECK, //check라는 액션을 할것이고
    payload: { result }, // 객체 안에 원하는 값이 있다는 모양으로 구현.
  };
}
export function next() {
  return {
    type: NEXT, //타입은 반드시 전달!
  };
}
...
export default function mbti(state = initState, action) {
  switch (
    action.type //액션에서 타입만 본다
  ) {
    case CHECK:
      return {
        ...state, //page등 건들지 않을 항목들 그대로 반환
        mbtiResult: state.mbtiResult + action.payload.result, //액션에 페이로드안에 result가 e,i,s,n... 을 반환, 그게 초기값 '' 뒤로 가서 붙는다
      };
    case NEXT:
      return {
        ...state,
        page: state.page + 1, //page넘기면 1씩 추가
      };
    case RESET:
      return {
        ...state,
        page: 0, //0페이지로 돌아가세요!
        mbtiResult: '', //초기값과 동일하게 맞춰준다
      };
    default:
      return state;
  }
